...
public class PointCountSeekbarPreference extends SeekBarPreference {
	private final int minimum = SettingRanges.minimumPointCount;
	private final int maximum = SettingRanges.maximumPointCount;

	public PointCountSeekbarPreference(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	protected int getPreferenceValue() {
		...
		int preferenceValue = preferences.getInt(SettingNames.pointCount, DefaultSettings.pointCount);

		return preferenceValue;
	}

	@Override
	protected void savePreferenceValue(int preferenceValue) {
		Editor editor = super.getEditor();
		editor.putInt(SettingNames.pointCount, preferenceValue);
